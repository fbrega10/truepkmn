...
impl ResponseError for PokeError {
    fn error_response(&self) -> HttpResponse {
        match self {
            PokeError::TimeoutError(_error) => HttpResponse::BadRequest().json(CustomError::new(ERROR_CODE_01,POKEMON_TIMEOUT)),
            PokeError::NotFound(_error) => HttpResponse::NotFound().json(CustomError::new(ERROR_CODE_02,POKEMON_NOT_FOUND)),
            PokeError::ServiceUnavailable(_error) => HttpResponse::ServiceUnavailable().json(CustomError::new(ERROR_CODE_03,SERVICE_UNAVAILABLE)),
        }
    }
}
